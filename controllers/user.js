    ...
    if(error){
        ...
        if(!recapchaVerifyResponse.data.success){
            ...
            userCollection.findOne({userEmail: userEmail}, (err, result)=>{
                if(err){
                    logger.log({level: 'error', message: 'Internal error for login user in database | Code: 1-2'})
                ...
                if(result == null){
                    ...
                    transporter.sendMail(mailOption, (err, data )=>{
                        if(err) {
                        logger.log({level: 'error', message: 'Internal error in node mailer send Mail function | Code: 1-3'})
                        return res.status(500).send({errorMessage: "Something went wrong when sent the mail."})
                        }
                    })
                    logger.log({level: 'info', message: 'User has been registered successfully. A link has been sent to user E-mail to verify user\'s account. | Code: 1-4'})
                    res.status(201).send({ userRegisterSuccessMessage: "User has been registered successfully. A link has been sent to your E-mail to verify your account."})
                }else{
                    logger.log({level: 'warn', message: 'Email already exist | Code: 1-5'})
...
const userVerifiedAccount = (req, res)=>{
    ...
    if (err) {
        logger.log({level: 'error', message: 'Internal error in database at the time of user verify account. | Code: 1-7'})
        return res.status(500).send({errorMessage: "Something went wrong"})
    }
  });
  logger.log({level:'info', message: 'User account has been verified successfully. | Code: 1-8'})
  res.status(200).send({ verifiedMessage: "Your account has been verified successfully."})
}
...
const mailForgetPasswordResetLink = (req, res)=>{
    ...
    userCollection.findOne({userEmail: userEmail},(err, user)=>{
        if(err) {
            logger.log({level: 'error', message: 'Internal error in database when user try to reset password. | code: 3-1'})
            return res.status(500).send({errorMessage: "Something went wrong"})
        }   
        if(user==null){
            logger.log({ level: 'error', message: 'Email is not registered. | code: 3-1'})
            ...
            transporter.sendMail(mailOption, (err, data )=>{
                if(err) {
                    logger.log({level: 'error', message: 'Internal error in node mailer send Mail function. | code: 3-3'})
                    return res.status(500).send({errorMessage: "Something went wrong"})
                }
            })
            logger.log({level: 'info', message: 'A reset password link has been sent to the mail of the user. | code: 3-4'})
            ...
            logger.log({level: 'warn', message: 'user cannot reset password. Because user logged in using google account. | code: 3-5'})
            ...
            logger.log({level: 'warn', message: 'user cannot reset password. Because user logged in using linkedIn account. | code: 3-6'})
...
const userForgetPassword = async(req, res)=>{
   ...
   if(error){
    ...
    userCollection.findOne({userEmail: userEmail},async(err, user)=>{
        if(err){
            logger.log({level: 'error', message: 'Internal error in user forget password. | code: 4-2'})
            return res.status(500).send({ errorMessage: "Something went wrong."})
        } 
        if(user==null){
            ...
            userCollection.updateOne(userInformation, updatedUserInformation, function(err, object) {
                ...
                logger.log({ level: 'info', message: 'User password has been updated successfully. | code: 4-4'})
...
const userUpdatePassword = (req, res)=>{
    ...
    if(error){
        ...
        userCollection.findOne({_id: ObjectId(userId)},async(err, user)=>{
            if(err){
                logger.log({level: 'error', message: 'Internal error in user update password function. | code: 5-2'})
                return res.status(500).send({errorMessage: "Something went wrong"})
            } 
            if(user==null){
...
const updateUserInformation = (req, res)=>{
    ...
    userCollection.findOne({userEmail: userEmailParam}, async (error, user)=>{
        if(error) {
            logger.log({level: 'error', message: 'Internal error in user update information function. | code: 6-2'})
            return res.status(500).send({errorMessage: "Something went wrong"})
        }
        if(user==null){
            logger.log({level: 'error', message: 'User is not available to update the information. | code: 6-3'})
            ...
            logger.log({level: 'info', message: 'user information has been updated successfully. | code: 6-4'})
...
const deleteSingleUser = (req, res)=>{
    ...
    userCollection.deleteOne(deletedUserId,(err,data)=>{
        if(err) {
            logger.log({level: 'error', message: 'Internal error in user delete function. | code: 8-1'})
            return res.status(500).send({errorMessage: "Something went wrong"})
        }
        logger.log({level: 'info', message: 'User has been deleted successfully. | code: 8-2'})
        res.status(200).send({
...
const getUserProfile = (req,res)=>{
    ...
    userCollection.find({ userEmail: userEmail}).toArray((err, result)=>{
        ...
        logger.log({level: 'info', message: 'Get all the info of requested user after login. | code: 9-1'})
...
const userRefreshToken = async(req, res)=>{
    ...
    if(redisUserEmail === null){
        logger.log({level: 'info', message: 'Login first before get refresh token. | code: 10-4'})
        ...
        try {
            ...
            logger.log({level: 'info', message: 'Authentication and refresh token have been sent. | code: 10-5'})
            res.status(200).send({authToken: authToken, refreshToken: refreshToken})          
        } catch (error) {
            logger.log({level: 'error', message: 'Internal error in refresh token function. | code: 10-6'})
...
const userLogOut = (req, res)=>{
    ...
    logger.log({level: 'info', message: 'User has been logged out successfully. | 12-1'})
